...
Agent 不是 Codex 桌面端，也不需要本地模型。它直接运行在扩展中，使用你在设置里配置的 OpenAI 兼容中转 API；支持 Responses 和 Chat Completions，两种协议都优先请求严格结构化 JSON。中转不支持结构化输出时，只会降级一次为普通 JSON，不会在 401、403、429、502 或网络错误时偷偷切换线路或反复扣费。
