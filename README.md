...
连续填写启用 Agent 后，会先收集所有可安全访问的页面，把每一页的完整题干、注释、日期示例、禁用字符、表头和控件结构组成同一份跨页蓝图，再统一决定科研训练、论文、竞赛、荣誉和项目经历应该分别放在哪一页。全局方案生成后，扩展会通过安全的“上一步”逐页返回起点并开始执行；若空白必填项阻止只读翻页，则安全降级为边填写边规划，不会尝试提交或绕过页面校验。

- 当前任务优先显示为“学校 · 院系”；页面没有可靠院系证据时显示“学校 · 项目 1/2/3”，也可由用户自定义名称并随时恢复自动名称；
- 本科院校、本科院系和本科专业被明确视为申请人来源资料，不会再被误用为目标学校或目标院系；证据不足时只允许 Agent 从页面真实文本中判断；
- 同一网站的不同报名项目按 URL 中明确的项目标识隔离，即使在同一标签页切换，也不会串用另一项目的页面记录和计划；
- 动态“新增记录”弹窗按照真实字段顺序逐条处理。确定性投影缺少必填值时，Agent 仅能基于当前单条资料补全当前弹窗字段，输出还要经过来源证据、字段白名单和写后回读校验；
- “识别并预览”与自动填写使用同一套页面级 Agent 输入，页面全文和字段注释不再只停留在扫描结果里；旧的“保守 / 标准 / 尽量填充”三档已移除；
- 网页标色只给真实控件加轮廓，并对嵌套长表格的题目单元格做可恢复的顶部对齐，避免新增多行后左侧题目被挤到表格底部。
...
Agent 会先区分整道题的用途，再检索资料，而不是看到“奖项”二字就混用所有获奖记录：

- “学术成果（包括荣获奖项、发表论文、学术活动等）”会综合科研训练、已发表论文、已取得专利和学科竞赛；单个长文本框会使用一份带逐项证据的综合资料，由模型按页面篇幅组织内容；
- “何时何地何原因受过何种奖励”或“奖励情况（本科期间）”只使用荣誉奖励，不再把科研项目和学科竞赛重复塞入该页；
- 明确写着“学科竞赛”“论文情况”或“专利情况”的表格仍各取对应资料，避免为了适配一个系统破坏其他学校。

例如某份本地荣誉资料保存为“获奖名称、等级、时间、主办单位、描述、排名”，而学校页面只有“时间、地点、内容”三列。Agent 会理解这一页的填写风格，再把每一条荣誉组织成完整的一行：

- 时间：按页面示例转换为 `YYYY-MM`、`YYYY年M月` 等格式；
- 地点：按目标列的语义选择可核实的授奖机构或地理地点；“校级、省级、市级”等级不会被当作地点，没有地点依据时留待确认，不凭空补城市；
- 内容：先判断记录是奖学金、荣誉称号、个人表彰还是团队荣誉，再自然组织成句，不套用固定括号模板；
- 时间、地点、内容会作为相邻列共同推理，避免同一事实在多列重复；资料只有年份、页面强制年月时会留待确认，不编造月份；
...
- 网页中已经上传的照片或材料会原样保留并加入预览门禁；字段没有独立标签时用当前页面题目命名。若学校服务器把图片错误标成通用二进制类型，扩展只有在文件头确认为受支持图片后才展示预览；
