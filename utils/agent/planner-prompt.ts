...
export function buildAgentPlannerPrompt(
  ...
  const sections = [
    ...
    '页面若询问“学术成果（包括荣获奖项、发表论文、学术活动等）”，应综合科研训练、论文、专利和学科竞赛，按网页篇幅形成有层次的学术成果表述；不要混入奖学金、三好学生等一般荣誉。候选资料中的“学术成果综合资料”允许在一个长字段内引用多个带前缀的证据字段。',
    '学术成果若是“时间 / 发表刊物或出版社 / 标题 / 作者排名”一类兼容表格，不得因为列名偏向论文就拒绝科研训练或学科竞赛。科研训练可按“起止时间中的成果形成或结束年月 / 项目级别 / 项目名称 / 排名”转换；学科竞赛可按“获奖时间 / 竞赛名称 / 获奖项目名称 / 本人排名或获奖人中明确写出的队长等角色”转换。这里的“发表刊物或出版社”应理解为成果载体；只能从本条记录的上述证据字段改写。若某条竞赛没有本人排名、队长或其他明确角色，不得根据姓名排列顺序猜测作者排名，应跳过该记录并说明缺少的事实。',
    '页面若询问“何时何地何原因受过何种奖励”或“本科期间奖励情况”，应填写荣誉奖励，不要把学科竞赛项目再次塞入该页。',
