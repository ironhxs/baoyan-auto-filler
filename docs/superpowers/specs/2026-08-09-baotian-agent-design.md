...
版本目标为 `2.0.0`。Agent 架构属于产品级跃迁，因此使用新的主版本号。
...
15. README、版本号、Git 标签和 Release 与 `2.0.0` 一致，不包含个人资料、API Key、登录状态或真实材料。

## 16. 发布边界

`2.0.0` 是 Agent 架构版本。此前尚未发布的 `1.5.1` 修复合并进入 `2.0.0`，不再单独创建 `v1.5.1` 或 `v1.6.0` Release。现有 `v1.5.0` 标签保持不变。
